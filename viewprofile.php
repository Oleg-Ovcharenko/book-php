<?php
  ...
  if (!isset($_SESSION['user_id'])) {
    echo '<p class="login">Please <a href="login.php">Войти</a> на свою страницу.</p>';
    exit();
  }
  else {
  ...
  if (mysqli_num_rows($data) == 1) {
    ...
    if (!empty($row['city']) || !empty($row['state'])) {
      echo '<tr><td class="label">Проживает:</td><td>' . $row['city'] . ', ' . $row['state'] . '</td></tr>';
    }
    ...
    echo '</table>';
    if (!isset($_GET['user_id']) || ($_SESSION['user_id'] == $_GET['user_id'])) {
      echo '<p>Вы можете <a href="editprofile.php">отредактировать свой профиль.</a></p>';
    }
